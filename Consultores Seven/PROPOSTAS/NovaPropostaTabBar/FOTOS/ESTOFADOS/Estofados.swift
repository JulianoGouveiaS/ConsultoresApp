    ...
    func loadFotos(){
        ...
        db.collection("ConsultorSeven").document("FotosPropostas").collection("\(self.id_user!)").document("\(self.propostaEscolhida.id!)").addSnapshotListener { (querySnapshot, err) in
            if let err = err {
                ...
                for i in 1...4{
                    if (dictionary?["esto\(i)_st"] as? String ?? "").contains("/emulated/"){
                        let arr1 = (dictionary?["esto\(i)_st"] as? String ?? "").components(separatedBy: "/Seven/")
        ...
        for image in images{
            if self.ArrFotosUrls[0] == ""{
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    self.enviaFotoStorage(nomeImg: "esto1_st", imagem: fotoComprimida!, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                })
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    self.enviaFotoStorage(nomeImg: "esto2_st", imagem: fotoComprimida!, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                })
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    self.enviaFotoStorage(nomeImg: "esto3_st", imagem: fotoComprimida!, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    self.enviaFotoStorage(nomeImg: "esto4_st", imagem: fotoComprimida!, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
