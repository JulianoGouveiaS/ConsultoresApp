...
class Estofados: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, GalleryControllerDelegate {
            ...
            if cell.imagem.image != nil{
                ...
                let defaultAction = CFAlertAction(title: "Excluir Foto",
                                                  ...
                                                  handler: { (action) in
                                                    ...
                                                    self.removeFoto(field: "esto\(indexPath.row+1)_st", proposta: self.propostaEscolhida!, id_user: "\(self.id_user!)")
    ...
    func loadFotos(){
        ...
        db.collection("ConsultorSeven").document("FotosPropostas").collection("\(self.id_user!)").document("\(self.propostaEscolhida.id!)").addSnapshotListener { (querySnapshot, err) in
            if let err = err {
                ...
                for i in 1...4{
                    if (dictionary?["esto\(i)_st"] as? String ?? "").contains(".jpg"){
                        self.ArrFotosUrls.append("https://firebasestorage.googleapis.com/v0/b/seven-app-61ea3.appspot.com/o/\(dictionary?["esto\(i)_st"] as? String ?? "")?alt=media")
                    }else{
            ...
            if self.ArrFotosUrls[0] == ""{
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    if let fotoComprimida = self.colocaLogo(imgData: data!).jpeg(.lowest) {
                        ...
                        self.enviaFotoStorage(nomeImg: "esto1_st", imagemDados: fotoComprimida2, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                        self.myCollectionView.reloadData()
                        }}
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    if let fotoComprimida = self.colocaLogo(imgData: data!).jpeg(.lowest) {
                        ...
                        self.enviaFotoStorage(nomeImg: "esto2_st", imagemDados: fotoComprimida2, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                        self.myCollectionView.reloadData()
                        }}
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    if let fotoComprimida = self.colocaLogo(imgData: data!).jpeg(.lowest) {
                        ...
                        self.enviaFotoStorage(nomeImg: "esto3_st", imagemDados: fotoComprimida2, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                        self.myCollectionView.reloadData()
                        }}
                })
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    if let fotoComprimida = self.colocaLogo(imgData: data!).jpeg(.lowest) {
                        ...
                        self.enviaFotoStorage(nomeImg: "esto4_st", imagemDados: fotoComprimida2, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                        self.myCollectionView.reloadData()
                        }}
                })
            }
