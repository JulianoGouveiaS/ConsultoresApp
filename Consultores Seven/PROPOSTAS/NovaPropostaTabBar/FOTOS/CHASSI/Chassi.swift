...
class Chassi: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, GalleryControllerDelegate {
    ...
    func loadFotos(){
        ...
        db.collection("ConsultorSeven").document("FotosPropostas").collection("\(self.id_user!)").document("\(self.propostaEscolhida.id!)").addSnapshotListener { (querySnapshot, err) in
            if let err = err {
                ...
                if (dictionary?["chassi_st"] as? String ?? "").contains("/emulated/"){
                    let arr1 = (dictionary?["chassi_st"] as? String ?? "").components(separatedBy: "/Seven/")
    ...
    func galleryController(_ controller: GalleryController, didSelectImages images: [Image]) {
        ...
        images[0].resolve(completion: { (imagem) in
            ...
            self.enviaFotoStorage(nomeImg: "chassi_st", imagem: fotoComprimida!, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
