...
class Dut: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, GalleryControllerDelegate {
            ...
            if cell.imagem.image != nil{
                ...
                let defaultAction = CFAlertAction(title: "Excluir Foto",
                                                  ...
                                                  handler: { (action) in
                                                    ...
                                                    self.removeFoto(field: "dut_st", proposta: self.propostaEscolhida!, id_user: "\(self.id_user!)")
    ...
    func loadFotos(){
        ...
        db.collection("ConsultorSeven").document("FotosPropostas").collection("\(self.id_user!)").document("\(self.propostaEscolhida.id!)").addSnapshotListener { (querySnapshot, err) in
            if let err = err {
                ...
                if (dictionary?["dut_st"] as? String ?? "").contains(".jpg"){
                    self.ArrFotosUrls.append("https://firebasestorage.googleapis.com/v0/b/seven-app-61ea3.appspot.com/o/\(dictionary?["dut_st"] as? String ?? "")?alt=media")
                }else{
    ...
    func galleryController(_ controller: GalleryController, didSelectImages images: [Image]) {
        ...
        images[0].resolve(completion: { (imagem) in
            ...
            if let fotoComprimida = self.colocaLogo(imgData: data1!).jpeg(.lowest) {
                ...
                self.enviaFotoStorage(nomeImg: "dut_st", imagemDados: fotoComprimida2, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                self.myCollectionView.reloadData()
                }}
