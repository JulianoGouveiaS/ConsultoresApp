...
class Lanterna: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, GalleryControllerDelegate {
    ...
    @objc func addFoto(){
          self.showGallery(ArrFotosUrls: self.ArrFotosUrls, totalFotos: 2, permissao: self.permissao)
    }
            ...
            if cell.imagem.image != nil{
                ...
                let defaultAction = CFAlertAction(title: "Excluir Foto",
                                                  ...
                                                  handler: { (action) in
                                                    ...
                                                    self.removeFoto(field: "lanterna\(indexPath.row+1)_st", proposta: self.propostaEscolhida!, id_user: "\(self.id_user!)")
    ...
    func loadFotos(){
        ...
        db.collection("ConsultorSeven").document("FotosPropostas").collection("\(self.id_user!)").document("\(self.propostaEscolhida.id!)").addSnapshotListener { (querySnapshot, err) in
            if let err = err {
                ...
                for i in 1...2{
                    if (dictionary?["lanterna\(i)_st"] as? String ?? "").contains(".jpg"){
                        self.ArrFotosUrls.append("https://firebasestorage.googleapis.com/v0/b/seven-app-61ea3.appspot.com/o/\(dictionary?["lanterna\(i)_st"] as? String ?? "")?alt=media")
                    }else{
            ...
            if self.ArrFotosUrls[0] == ""{
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    if let fotoComprimida = self.colocaLogo(imgData: data!).jpeg(.lowest) {
                        ...
                        self.enviaFotoStorage(nomeImg: "lanterna1_st", imagemDados: fotoComprimida2, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                        self.myCollectionView.reloadData()
                        }
                    }
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    if let fotoComprimida = self.colocaLogo(imgData: data!).jpeg(.lowest) {
                        if let fotoComprimida2 = self.colocaLogo(imgData: fotoComprimida).jpeg(.lowest) {
                            ...
                            self.enviaFotoStorage(nomeImg: "lanterna2_st", imagemDados: fotoComprimida2, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                            self.myCollectionView.reloadData()
                        }
                    }
