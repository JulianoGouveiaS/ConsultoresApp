    ...
    func loadFotos(){
        ...
        db.collection("ConsultorSeven").document("FotosPropostas").collection("\(self.id_user!)").document("\(self.propostaEscolhida.id!)").addSnapshotListener { (querySnapshot, err) in
            if let err = err {
                ...
                for i in 1...2{
                    if (dictionary?["lanterna\(i)_st"] as? String ?? "").contains("/emulated/"){
                        let arr1 = (dictionary?["lanterna\(i)_st"] as? String ?? "").components(separatedBy: "/Seven/")
        ...
        for image in images{
            if self.ArrFotosUrls[0] == ""{
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    self.enviaFotoStorage(nomeImg: "lanterna1_st", imagem: fotoComprimida!, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
                ...
                image.resolve(completion: { (imagem) in
                    ...
                    self.enviaFotoStorage(nomeImg: "lanterna2_st", imagem: fotoComprimida!, id_user: "\(self.id_user!)", proposta: self.propostaEscolhida)
